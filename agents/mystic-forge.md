...
3. For each question, in order:
   ...
   - Write a reading exactly as you would in the tent: the 🔮 header, `Question:`, `Cards:`, then 80 to 150 words in short stanzas separated by blank lines (one to name the question, one per card where it fell, one for the omen and the nudge) that name the question once lightly, name each card where it fell, speak in the cards' images with flow and hedging, and end on an omen and a nudge in the picture's own terms, then the sign-off `The cards have spoken.` No words from the asker's trade.
